$(document).ready(function() {     
    ...
    $("#btnAgregarCarritoVentas").click(function(){ 
        ...
        if(parseFloat($('#cantidad').val()) > parseFloat($("#stock").val())){
            alertify.alert("¡Cantidad fuera de rango!", function(){
                alertify.message('La cantidad no puede superar el Stock');
            });
            $("#cantidad").focus();
        }
        else
...
//aqui venia cierre de document(ready)
...
//--  INICIO FORMAS DE PAGO --//
    ...
    $("#btnAplicarFormaPago").click(function(){
            if(idFormaPago !== 0 && idFormaPago !== 8 ){
				...
				tarjetaDebito = $('#tarjetaDebito option:selected').val(); 
                tarjetaCredito = $('#tarjetaCredito option:selected').val(); 
...
//INICIO otras OFP (otras formas de pago)
	...
	$("#nroNC").val(""); //inicializo el input	
	$("#importeNC").val(""); //inicializo el input
		 ...
		 $.ajax({
			  ...
			  success: function(data) {
					...
					if (!$.trim(datos)){
						...
						//$("#txtTotalVenta").val(parseFloat(totDevolPositiva).toFixed(2));
						...
						if ($("#estadoNC").val() == "Cancelada"){
							alertify.warning("La NC ya fue CANCELADA");
							$("#formasDePagoEDC").prop('disabled', true);
							$("#btnAplicarOFP").prop('disabled', true);
							$("#nroNC").focus();
						}else{
